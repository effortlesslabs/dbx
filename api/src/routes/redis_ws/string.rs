...
async fn handle_redis_ws_string_socket(socket: WebSocket, pool: Arc<RedisPool>) {
    ...
    while let Some(Ok(msg)) = receiver.next().await {
        if let axum::extract::ws::Message::Text(text) = msg {
            if let Ok(message) = serde_json::from_str::<StringWsMessage>(&text) {
                ...
                match message {
                    ...
                    StringWsMessage::BatchGet { keys } => {
                        ...
                        let _ = sender.send(
                            axum::extract::ws::Message::Text(
                                serde_json
                                    ::to_string(&(StringWsMessage::BatchResult { keys, values }))
                                    .unwrap()
                            )
                        ).await;
                    }
                        ...
                        let msg = match res {
                            Ok(_) =>
                                StringWsMessage::Result {
                                    key: "batch".to_string(),
                                    value: Some(
                                        format!("Successfully set {} operations", operations.len())
                                    ),
                                },
