...
use dbx_crates::adapter::redis::primitives::admin::{ HealthCheck, ServerStatus };
...
pub enum AdminWsMessage {
    ...
    // Statistics messages
    #[serde(rename = "memory_stats")] MemoryStats,
    #[serde(rename = "client_stats")] ClientStats,
    #[serde(rename = "server_stats")] ServerStats,

    // Configuration messages
    #[serde(rename = "config_set")] ConfigSet {
        parameter: String,
        value: String,
    },
    #[serde(rename = "config_get")] ConfigGet {
        parameter: String,
    },
    #[serde(rename = "config_get_all")] ConfigGetAll,
    #[serde(rename = "config_resetstat")] ConfigResetStat,
    #[serde(rename = "config_rewrite")] ConfigRewrite,
    ...
    // Response messages
    #[serde(rename = "ping_result")] PingResult {
        response: String,
    },
    #[serde(rename = "info_result")] InfoResult {
        info: String,
    },
    #[serde(rename = "dbsize_result")] DbSizeResult {
        size: i64,
    },
    #[serde(rename = "time_result")] TimeResult {
        seconds: i64,
        microseconds: i64,
    },
    #[serde(rename = "version_result")] VersionResult {
        version: String,
    },
    #[serde(rename = "health_result")] HealthResult {
        health: HealthCheck,
    },
    #[serde(rename = "status_result")] StatusResult {
        status: ServerStatus,
    },
    #[serde(rename = "memory_stats_result")] MemoryStatsResult {
        stats: HashMap<String, String>,
    },
    #[serde(rename = "client_stats_result")] ClientStatsResult {
        stats: HashMap<String, String>,
    },
    #[serde(rename = "server_stats_result")] ServerStatsResult {
        stats: HashMap<String, String>,
    },
    #[serde(rename = "config_get_result")] ConfigGetResult {
        parameter: String,
        value: String,
    },
    #[serde(rename = "config_get_all_result")] ConfigGetAllResult {
        config: HashMap<String, String>,
    },
    #[serde(rename = "config_set_result")] ConfigSetResult {
        parameter: String,
        value: String,
    },
    #[serde(rename = "config_resetstat_result")] ConfigResetStatResult,
    #[serde(rename = "config_rewrite_result")] ConfigRewriteResult,
    #[serde(rename = "flushdb_result")] FlushDbResult,
    #[serde(rename = "flushall_result")] FlushAllResult,
    ...
    while let Some(Ok(msg)) = receiver.next().await {
        if let axum::extract::ws::Message::Text(text) = msg {
            if let Ok(message) = serde_json::from_str::<AdminWsMessage>(&text) {
                    ...
                    Err(e) => {
                        let _ = sender.send(
                            axum::extract::ws::Message::Text(
                                serde_json
                                    ::to_string(
                                        &AdminWsMessage::Error(format!("Redis error: {}", e))
                                    )
                                    .unwrap()
                            )
                        ).await;
                ...
                match message {
                    AdminWsMessage::Ping => {
                        let response = ping_server(conn_arc.clone()).unwrap_or_else(|_|
                            "ERROR".to_string()
                        );
                        let _ = sender.send(
                            axum::extract::ws::Message::Text(
                                serde_json
                                    ::to_string(&(AdminWsMessage::PingResult { response }))
                                    .unwrap()
                            )
                        ).await;
                    }
                    AdminWsMessage::Info { section } => {
                        let info = if let Some(section) = section {
                            get_server_info_section(conn_arc.clone(), &section).unwrap_or_else(|_|
                                "ERROR".to_string()
                            )
                        } else {
                            get_server_info(conn_arc.clone()).unwrap_or_else(|_|
                                "ERROR".to_string()
                            )
                        };
                        let _ = sender.send(
                            axum::extract::ws::Message::Text(
                                serde_json
                                    ::to_string(&(AdminWsMessage::InfoResult { info }))
                                    .unwrap()
                            )
                        ).await;
                    }
                    AdminWsMessage::DbSize => {
                        ...
                        let _ = sender.send(
                            axum::extract::ws::Message::Text(
                                serde_json
                                    ::to_string(&(AdminWsMessage::DbSizeResult { size }))
                                    .unwrap()
                            )
                        ).await;
                    }
                    ...
                    AdminWsMessage::Version => {
                        let version = get_server_version(conn_arc.clone()).unwrap_or_else(|_|
                            "UNKNOWN".to_string()
                        );
                        let _ = sender.send(
                            axum::extract::ws::Message::Text(
                                serde_json
                                    ::to_string(&(AdminWsMessage::VersionResult { version }))
                                    .unwrap()
                            )
                        ).await;
                    }
                    AdminWsMessage::Health => {
                        let health = health_check(conn_arc.clone()).unwrap_or_else(|_| HealthCheck {
                            is_healthy: false,
                        ...
                        let _ = sender.send(
                            axum::extract::ws::Message::Text(
                                serde_json
                                    ::to_string(&(AdminWsMessage::HealthResult { health }))
                                    .unwrap()
                            )
                        ).await;
                    }
                    AdminWsMessage::Status => {
                        let status = server_status(conn_arc.clone()).unwrap_or_else(
                            |_| ServerStatus {
                                timestamp: 0,
                                uptime_seconds: 0,
                                connected_clients: 0,
                        ...
                        let _ = sender.send(
                            axum::extract::ws::Message::Text(
                                serde_json
                                    ::to_string(&(AdminWsMessage::StatusResult { status }))
                                    .unwrap()
                            )
                        ).await;
                    }
                    AdminWsMessage::MemoryStats => {
                        ...
                        let _ = sender.send(
                            axum::extract::ws::Message::Text(
                                serde_json
                                    ::to_string(&(AdminWsMessage::MemoryStatsResult { stats }))
                                    .unwrap()
                            )
                        ).await;
                    }
                    AdminWsMessage::ClientStats => {
                        ...
                        let _ = sender.send(
                            axum::extract::ws::Message::Text(
                                serde_json
                                    ::to_string(&(AdminWsMessage::ClientStatsResult { stats }))
                                    .unwrap()
                            )
                        ).await;
                    }
                    AdminWsMessage::ServerStats => {
                        ...
                        let _ = sender.send(
                            axum::extract::ws::Message::Text(
                                serde_json
                                    ::to_string(&(AdminWsMessage::ServerStatsResult { stats }))
                                    .unwrap()
                            )
                        ).await;
                    }
                    ...
                    AdminWsMessage::ConfigGet { parameter } => {
                        let value = config_get(conn_arc.clone(), &parameter).unwrap_or_else(|_|
                            "ERROR".to_string()
                        );
                        let _ = sender.send(
                            axum::extract::ws::Message::Text(
                    ...
                    AdminWsMessage::ConfigGetAll => {
                        ...
                        let _ = sender.send(
                            axum::extract::ws::Message::Text(
                                serde_json
                                    ::to_string(&(AdminWsMessage::ConfigGetAllResult { config }))
                                    .unwrap()
                            )
                        ).await;
                    }
...
pub fn create_redis_ws_admin_routes(pool: Arc<RedisPool>) -> Router {
    Router::new().route("/admin/ws", get(redis_ws_admin_handler)).with_state(pool)
}
