...
impl WebSocketCommandProcessor {
    ...
    pub async fn process_command(&self, ws_message: WebSocketMessage) -> WebSocketResponse {
        ...
        match ws_message.command {
            ...
            WebSocketCommand::Delete { key } => self.handle_delete_command(command_id, key).await,
            WebSocketCommand::Exists { key } => self.handle_exists_command(command_id, key).await,
            ...
            WebSocketCommand::IncrBy { key, increment } => {
                self.handle_incr_by_command(command_id, key, increment)
                    .await
            }
            WebSocketCommand::SetNx { key, value, ttl } => {
                self.handle_set_nx_command(command_id, key, value, ttl)
                    .await
            }
            WebSocketCommand::CompareAndSet {
                key,
                expected_value,
                new_value,
                ttl,
            } => {
                self.handle_compare_and_set_command(command_id, key, expected_value, new_value, ttl)
                    .await
            }
            ...
            WebSocketCommand::Sdiff { keys } => self.handle_sdiff_command(command_id, keys).await,
            WebSocketCommand::Sinter { keys } => self.handle_sinter_command(command_id, keys).await,
            WebSocketCommand::Sunion { keys } => self.handle_sunion_command(command_id, keys).await,
            WebSocketCommand::Smove {
                source,
                destination,
                member,
            } => {
                self.handle_smove_command(command_id, source, destination, member)
                    .await
            }

            // Hash commands
            WebSocketCommand::Hset { key, field, value } => {
                self.handle_hset_command(command_id, key, field, value)
                    .await
            }
            ...
            WebSocketCommand::Hgetall { key } => self.handle_hgetall_command(command_id, key).await,
            ...
            WebSocketCommand::Keys { pattern } => {
                self.handle_keys_command(command_id, pattern.unwrap_or_else(|| "*".to_string()))
                    .await
            }
            ...
            WebSocketCommand::BatchSet { key_values, ttl } => {
                self.handle_batch_set_command(command_id, key_values, ttl)
                    .await
            }
            ...
            WebSocketCommand::BatchIncrBy { key_increments } => {
                self.handle_batch_incr_by_command(command_id, key_increments)
                    .await
            }
