...
impl RedisHandler {
    ...
    pub async fn hash_field_exists(
        ...
        match handler.redis.hash().hexists(&key, &field) {
    ...
    pub async fn set_hash_field_nx(
        ...
        match handler.redis.hash().hsetnx(&key, &field, &request.value) {
    ...
    pub async fn get_hash_values(
        ...
        match handler.redis.hash().hvals(&key) {
            Ok(values) => {
    ...
    pub async fn batch_set_hash_fields(
        ...
        match handler.redis.hash().hset_many(hash_fields) {
            Ok(results) => {
    ...
    pub async fn batch_get_hash_fields(
        ...
        match handler.redis.hash().hget_many(hash_fields) {
            Ok(values) => {
                let string_values: Vec<StringValue> = values
                    .into_iter()
                    .map(|v| StringValue {
    ...
    pub async fn batch_delete_hash_fields(
        ...
        match handler.redis.hash().hdel_many(hash_fields) {
            Ok(results) => {
    ...
    pub async fn batch_check_hash_fields(
        ...
        match handler.redis.hash().hexists_many(hash_fields) {
            Ok(results) => {
                let boolean_values: Vec<BooleanValue> = results
                    .into_iter()
                    .map(|r| BooleanValue { value: r })
