...
```bash
curl http://localhost:8080/api/admin/memory/user:123
```
...
```bash
curl -X POST http://localhost:8080/api/admin/flush
```
...
Use the `/api/admin/health` endpoint for load balancer health checks:

```bash
# Nginx configuration example
location /health {
    proxy_pass http://localhost:8080/api/admin/health;
    access_log off;
}
```
...
DBX exposes metrics in Prometheus format at `/api/admin/metrics`:
...
# Health check script
#!/bin/bash
response=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8080/api/admin/health)
...
curl -s http://localhost:8080/api/admin/stats | jq '.performance.average_response_time'

# Monitor error rates
curl -s http://localhost:8080/api/admin/stats | jq '.errors.rate'
```
...
```bash
# Check memory usage
curl -s http://localhost:8080/api/admin/health | jq '.memory.used'

# Check database connections
curl -s http://localhost:8080/api/admin/stats | jq '.database.connection_pool'
```
