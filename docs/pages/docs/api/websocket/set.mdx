...
DBX provides real-time set operations through WebSocket connections, allowing you to work with Redis set data structures efficiently.
...
// Add members to set
const added = await client.set.sadd("users:online", ["user:123", "user:456"]);

// Remove members from set
const removed = await client.set.srem("users:online", ["user:123"]);

// Get all members
const members = await client.set.smembers("users:online");

// Check if member exists
const exists = await client.set.sismember("users:online", "user:123");

// Get set size
const size = await client.set.scard("users:online");

// Get random member
const randomMember = await client.set.srandmember("users:online", 1);

// Remove random member
const popped = await client.set.spop("users:online", 1);

// Move member between sets
const moved = await client.set.smove("users:online", "users:away", "user:123");

// Set intersection
const intersection = await client.set.sinter(["users:online", "users:premium"]);

// Set union
const union = await client.set.sunion(["users:online", "users:away"]);

// Set difference
const difference = await client.set.sdiff(["users:online", "users:premium"]);

// Store operations
const cardinality = await client.set.sinterstore("result", ["set1", "set2"]);
