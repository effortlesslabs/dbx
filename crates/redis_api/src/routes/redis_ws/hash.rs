...
pub enum HashWsMessage {
    ...
    #[serde(rename = "all_result")] AllResult {
        key: String,
            ...
            if let Ok(message) = serde_json::from_str::<HashWsMessage>(&text) {
                    ...
                    Err(e) => {
                        let _ = sender.send(
                            axum::extract::ws::Message::Text(
                                serde_json
                                    ::to_string(
                                        &HashWsMessage::Error(format!("Redis error: {}", e))
                                    )
                                    .unwrap()
                            )
                        ).await;
                ...
                match message {
                    HashWsMessage::Get { key, field } => {
                        let value = get_hash_field(conn_arc.clone(), &key, &field).ok().flatten();
                        let _ = sender.send(
                            axum::extract::ws::Message::Text(
                                serde_json
                                    ::to_string(
                                        &(HashWsMessage::Result { key, field: Some(field), value })
                                    )
                                    .unwrap()
                            )
                        ).await;
                    ...
                    HashWsMessage::Del { key, field } => {
                        let deleted = delete_hash_field(conn_arc.clone(), &key, &field).unwrap_or(
                            false
                        );
                        let _ = sender.send(
                            axum::extract::ws::Message::Text(
                                serde_json
                                    ::to_string(&(HashWsMessage::Deleted { key, field, deleted }))
                                    .unwrap()
                            )
                        ).await;
                    }
                    HashWsMessage::GetAll { key } => {
                        let fields = get_all_hash_fields(
                            conn_arc.clone(),
                            &key
                        ).unwrap_or_default();
                        let _ = sender.send(
                            axum::extract::ws::Message::Text(
                                serde_json
                                    ::to_string(&(HashWsMessage::AllResult { key, fields }))
                                    .unwrap()
                            )
                        ).await;
                    }
...
pub fn create_redis_ws_hash_routes(pool: Arc<RedisPool>) -> Router {
    Router::new().route("/hash/ws", get(redis_ws_hash_handler)).with_state(pool)
}
