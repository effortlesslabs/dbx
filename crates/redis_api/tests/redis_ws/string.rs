...
async fn test_redis_ws_batch_string_operations() {
    ...
    let batch_set_response = send_message_and_get_response(
        &mut write,
        &mut read,
        batch_set_message
    ).await;
    ...
    let batch_get_response = send_message_and_get_response(
        &mut write,
        &mut read,
        batch_get_message
    ).await;
    assert_eq!(batch_get_response["type"], "batch_result");
    assert_eq!(batch_get_response["data"]["keys"].as_array().unwrap().len(), 3);
    assert_eq!(batch_get_response["data"]["values"].as_array().unwrap().len(), 3);
}
...
async fn test_redis_ws_concurrent_string_operations() {
    ...
    for i in 0..5 {
        let handle = tokio::spawn(async move {
            ...
            let set_response = send_message_and_get_response(
                &mut write,
                &mut read,
                set_message
            ).await;
            ...
            let get_response = send_message_and_get_response(
                &mut write,
                &mut read,
                get_message
            ).await;
            ...
            let del_response = send_message_and_get_response(
                &mut write,
                &mut read,
                del_message
            ).await;
